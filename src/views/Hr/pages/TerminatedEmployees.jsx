...
import { fetchTerminatedEmployees } from "../../../services/api";
...
const TerminatedEmployees = () => {
  ...
  const {
    data: terminatedEmployess,
    error: terminatedError,
    isLoading: terminatedLoading,
    refetch: refetchTerminated,
  } = useQuery("terminatedEmployees", fetchTerminatedEmployees);
  ...
  const {
    ...
    ["filteredTerminatedEmployees", filters],
  ...
  const fetchFilteredEmployees = async (filters) => {
    try {
      setIsFetching(true);
      const response = await axios.post(`${API}/api/terminated-employees`, filters);
  ...
  const currentEmployees = filteredEmployees
    ...
    : terminatedEmployess
    ? terminatedEmployess.slice(
        currentPage * employeesPerPage,
        (currentPage + 1) * employeesPerPage
      )
    : [];

  if (terminatedLoading) return <Loading />;
  if (terminatedError) return <Error message={terminatedError.message} />;
  ...
  const handleExportToExcel = () => {
    exportToExcel(exportData, "FilteredTerminatedEmployees.xlsx");
  };

  const exportData2 = flattenDeployedEmployeesForReport(terminatedEmployess);
  const handleExportAllToExcel = () => {
    exportToExcel(exportData2, "AllTerminatedEmployees.xlsx");
  };
  ...
  return (
    <>
      <BreadCrumb title={"Terminated Employees"} activeTab={"Terminated"} />
                ...
                <ReactPaginate
                  ...
                  pageCount={
                    filteredEmployees
                      ...
                      : Math.ceil(terminatedEmployess.length / employeesPerPage)
                  }
